...
function netAfterTradeTax(grossDiamonds, taxRate) {
  const gross = ceilQty(grossDiamonds);
  return gross - tradeTaxAmount(gross, taxRate);
}

function requiredTradeGrossForNet(netDiamonds, taxRate) {
  const targetNet = ceilQty(netDiamonds);
  if (targetNet <= 0) return 0;
  let gross = Math.max(0, Math.floor(targetNet / (1 - taxRate)) - 2);
  while (netAfterTradeTax(gross, taxRate) < targetNet) gross += 1;
  while (gross > 0 && netAfterTradeTax(gross - 1, taxRate) >= targetNet) gross -= 1;
  return gross;
}

function tradeTaxAmount(grossDiamonds, taxRate) {
  const gross = ceilQty(grossDiamonds);
  return Math.floor(gross * taxRate + 1e-9);
}
